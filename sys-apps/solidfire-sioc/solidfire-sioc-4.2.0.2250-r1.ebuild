...
src_prepare()
{
    ...
    # Unfortunately, there's no way to disable the plugin through maven directly. So instead we change
    # the default phase of the plugin from 'validate' to 'none' which will prevent it from auto incrementing
    # the build.
    einfo "Disabling buildnumber-maven-plugin"
    perl -0777 -p -e 's|(<artifactId>buildnumber-maven-plugin</artifactId>)(.*)<phase>validate</phase>|\1\2<phase>none</phase>|s' -i pom.xml
}
...
src_compile()
{
    ...
    grep "buildNumber=${BUILD}" buildNumber.properties || die "buildNumber.properties has unexpected value."

    # Ensure no remnants of incorrect version number
    local next_build=$(( BUILD + 1 ))
    if grep -aR "${MY_PV}.${next_build}" .; then
        die "Found unexpected version number ${MY_PV}.${next_build}"
    fi
    
    if find . | grep "${MY_PV}.${next_build}"; then
        die "Found unexpected version number ${MY_PV}.${next_build}"
    fi
