...
src_install()
{
	...
	mkdir -p "${D}/etc/nginx.conf.d" "${D}/etc/nginx.legacy.conf.d" || die
	cp "files/etc/nginx.conf.d/upgrade_cluster.conf" "${D}/etc/nginx.conf.d" || die
	cp "files/etc/nginx.legacy.conf.d/upgrade_node.conf" "${D}/etc/nginx.legacy.conf.d" || die
