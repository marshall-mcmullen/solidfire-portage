...
src_install()
{
	...
	# /sf/packages/${PF}/confrestapp-${PVR}
	# And have a non-versioned symlink at /sf/packages/${PF}/confrestapp that they can then change to point to other
	# versions.
	mkdir -p "${DP}/confrestapp-${PVR}" || die
	cp --preserve=mode --recursive "${S}/." "${DP}/confrestapp-${PVR}" || die
	dosym "${PREFIX}/confrestapp-${PVR}" "${PREFIX}/confrestapp"
	if [[ ! -x "${DP}/confrestapp-${PVR}/confrestapp" ]]; then
		die "${DP}/confrestapp-${PVR}/confrestapp must be executable"
	fi
	...
	dosym "${PREFIX}/confrestapp-${PVR}/install/sf/hci/nde_reset.py" "/sf/hci/nde_reset.py"
