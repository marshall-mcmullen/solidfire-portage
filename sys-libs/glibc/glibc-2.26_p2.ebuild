...
# PE-3236: The fix itself is nptl-cond-broadcast-fix-loaded-refs-value.patch. This new ebuild is the same as 
# glibc-2.26-r7 with the exception of the new patch we've added. We can't name this package with -solidfire suffix like
# we normally like to with our customizations as lots of packages explicitly depend on 'glibc' package. But we add our
# _p* version suffix and we explicitly qualify our dependency in packages.json to sys-libs/glibc::solidfire so that
# we are ensured to be pulling in our custom version from our portage overlay.

# Strip off SolidFire version suffix
MY_PV=${PV%_p*}
MY_P=${P%_p*}
S="${WORKDIR}/${MY_P}"
...
PATCHES=(
	"${FILESDIR}/nptl-cond-broadcast-fix-loaded-refs-value.patch"
	"${FILESDIR}/nptl-rdlock-refresh-readers.patch"
)

SRC_URI+=" https://dev.gentoo.org/~dilfridge/distfiles/${MY_P}-patches-${PATCH_VER}.tar.bz2"
