...
GFLAGS_VERSION="solidfire-2.0_p5"
GTEST_VERSION="solidfire-1.6.0_p7"
GMOCK_VERSION="solidfire-1.6.0_p4"
GLOG_VERSION="solidfire-0.3.3_p3"
SRC_URI="http://bitbucket.org/solidfire/gflags/get/${GFLAGS_VERSION/-//}.tar.bz2 -> gflags-${GFLAGS_VERSION}.tar.bz2
         http://bitbucket.org/solidfire/gtest/get/${GTEST_VERSION/-//}.tar.bz2   -> gtest-${GTEST_VERSION}.tar.bz2
		 http://bitbucket.org/solidfire/gmock/get/${GMOCK_VERSION/-//}.tar.bz2   -> gmock-${GMOCK_VERSION}.tar.bz2
		 http://bitbucket.org/solidfire/glog/get/${GLOG_VERSION/-//}.tar.bz2     -> glog-${GLOG_VERSION}.tar.bz2"
...
src_prepare()
{
	append-cxxflags "-fpermissive -I${S}/gflags-${GFLAGS_VERSION}/src -I${S}/gtest-${GTEST_VERSION}/include -I${S}/gmock-${GMOCK_VERSION}/include -I${S}/glog-${GLOG_VERSION}/src"
	...
	for module in ${MODULES}; do
		...
		if [[ ${module} == @(gtest|gmock) ]]; then
			sed -i -e "s|# This library was specified with -dlpreopen.|if [[ \"\${name}\" -eq '${module}' ]]; then name='${module}-solidfire-${PV}'; fi\n    # This library was specified with -dlpreopen.|" \
				libtool || die

			sed -i -e "s|lib/lib${module}.la|lib/${PF}/lib${PF}.la|g"                                 \
				   -e "s|${module}_libs=\"-l\${name}|${module}_libs=\"-l${module}-solidfire-${PV}|g" \
...
src_install()
{
	...
	# Create eselect modules file so that eselect can manage exports, lib and include links properly
	mkdir -p "${DP}/eselect"
	echo "${MODULES}" >> "${DP}/eselect/modules"
}
...
src_test()
{
	...
	local cmd="g++ ${CXXFLAGS} ${test_cpp} -o ${test_binary} ${LDFLAGS} -Wl,--rpath=${S}/gflags-${GFLAGS_VERSION}/.libs -Wl,--rpath=${S}/glog-${GLOG_VERSION}/.libs -lgflags-solidfire-${PV} -lgtest-solidfire-${PV} -lgmock-solidfire-${PV} -lglog-solidfire-${PV}"
